...
const CartWidget = () => {
  ...
  const {counter,onEvent, posCart,toggleCartContainer} = useContext(CartContext)

    return (
        <>
        <div className="widget" style={{position:"relative"}} >
            <button className="btnCart" type="button" onClick={toggleCartContainer}>
              ...
              <span className="badge bg-primary text-center" style={{width:"1.7em", height: "1.7em", borderRadius: "100px", position: "absolute", right:"0px", top:"10px"}}>{counter<=0?null:counter}</span>
            </button>
        </div>
        </>
    )
